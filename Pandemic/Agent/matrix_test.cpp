...
TEST_CASE("Matrix Struct") {
  SUBCASE("Constructor") {
    SUBCASE("Parametric") {
      ...
      CHECK_THROWS_WITH(
          Matrix<int> parametric3(other),
          "Error! The matrix, you're trying to copy, isn't squared");

      CHECK_THROWS_AS(Matrix<int> parametric4(0, 1), std::runtime_error);
      CHECK_THROWS_WITH(Matrix<int> parametric4(0, 1),
  ...
  SUBCASE("Members functions") {
    ...
    CHECK_THROWS_WITH(Matrix<int> assign1(empty1),
    ...
    CHECK_THROWS_WITH(
        Matrix<int> assign1_ = not_square_,
        "Error! The matrix, you're trying to copy, isn't squared");

    //////////////////operator=/////////////////

    std::vector<std::vector<int>> square(
        17, {0, 1, 9, 2, 8, 3, 7, 4, 5, 6, 7, 4, 3, 2, 2, 1, 34});
