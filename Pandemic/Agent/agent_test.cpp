...
void check_state(Agent& a) {
  ...
  a.evolve(next);
  bool state;
  a.get_matrix().inside_matrix([previous, &state](Person& cell, std::size_t r,
                                                  std::size_t c) {
    ////////////Checking each cell///////////////
  ...
  SUBCASE("Constructors") {
    ...
    SUBCASE("Parametric") {
      ...
      SUBCASE("Member Functions") {
        ...
        ag1->sorting();
        std::cout << "Somma dei suscettibili:"
                  << sum(ag1->get_evolution().back().S_) << '\n';
        std::cout << "I non vaccinati :" << ag1->get_evolution().back().S_[0]
                  << '\n';
        std::cout << "I vaccinati :" << ag1->get_evolution().back().S_[1]
                  << '\n';
        CHECK(static_cast<double>(ag1->get_situation_day(2).S_[0]) /
                  (sum(ag1->get_situation_day(2).S_)) ==
              doctest::Approx(0.6).epsilon(0.1));
        CHECK(static_cast<double>(ag1->get_situation_day(2).S_[1]) /
                  (sum(ag1->get_situation_day(2).S_)) ==
        ...
        ag1->get_matrix().inside_matrix([&ag1](Person& cell, std::size_t r,
                                               std::size_t c) {
