...
int main() {
  ...
  while (sim != 'D' && sim != 'P') {
    ...
    if (it == MAX_IT) {
      std::cout
          << " The attempts are finished! It'll start the default simulation"
          << "\n\n";
      sim = 'D';
      break;
    }
  ...
  if (sim == 'D') {
    ...
    if (start == 'y') {
      ...
      //////////////Chance of comparison with a simulation with
      ///vaccine////////////////
      it = 0;
      char comparison;
      MAX_IT = 5;
      std::cout << "Would you be interested to observe the difference between "
                   "the last simulation and another with the option to get "
                   "vaccinated?[y/n]"
                << '\n';
      ...
      while ((comparison != 'y' && comparison != 'n')) {
        ...
        it++;
        if (it == MAX_IT) {
          std::cout << " The attempts are finished! Let's continue with the "
                       "comparison"
                    << "\n\n";
          comparison = 'y';
          break;
      ...
      if (comparison == 'y') {
        std::cout << "Great! Let's start another simulation! The initial "
    ...
    std::cout
        ...
        << "\n\n";
    std::cout << "In order : Susceptiple, Infected, Healed and Dead (the last "
                 "two must be null)"
              << '\n';
    std::cin >> people;
    ...
    if (duration_ == 'A') {
      ...
      while (!(std::cin >> comparison2) ||
             (comparison2 != 'y' && comparison2 != 'n')) {
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
      ...
      if (total(eq_->get_evolution().back().I_) > 0) {
        ...
        if (finish == 'y') {
          double vaccine;
          std::cout << "Insert the probability to get vaccinated [0,1], if you "
                       "want the option, otherwise just insert 0, please"
                    << '\n';
          std::cin >> vaccine;
          ...
          if (vaccine != 0) {
            ...
            while (!input_v2) {
              try {
                ...
                std::cout << "Error: " << e.what() << "\n\n";
                std::cout << "Please correct your data, according to the "
                             "error, and insert them in the same order"
                          << '\n';
                /////////////////Cleaning the in stream///////////
          ...
          std::cout << "Alright, thank you for your time! Here below, you'll "
                       "find the results of your simulation"
                    << '\n';
        }
