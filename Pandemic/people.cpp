...
People::People(const std::array<int, 2>& s, const std::array<int, 2>& i,
...
bool operator==(const People& left, const People& right) {
  return (left.S_[0] == right.S_[0] && left.S_[1] == right.S_[1] &&
          left.I_[0] == right.I_[0] && left.I_[1] == right.I_[1] &&
          left.H_ == right.H_ && left.D_ == right.D_);
}

std::ostream& operator<<(std::ostream& os, const People& p_out) {
  os << "Suscepttible not vaccinated : " << p_out.S_[0]
     << "||  Suscepttible vaccinated : " << p_out.S_[1] << "\n\n";
  os << "Infected not vaccinated: " << p_out.I_[0]
     << "|| Infected not vaccinated: " << p_out.I_[1] << "\n\n";
  os << " Healed : " << p_out.H_ << "\n\n";
  os << "Dead : " << p_out.D_ << "\n\n";
